...
ymin = abs(S.psi_p).min()
ymax = abs(S.psi_p).max()
ax2 = fig.add_subplot(212, xlim=plim,
                      ylim=(ymin - 0.2 * (ymax - ymin),
                            ymax + 0.2 * (ymax - ymin)))
psi_p_line, = ax2.plot([], [], c='r', label=r'$|\psi(p)|$')

p0_line1 = ax2.axvline(-p0, c='k', ls=':', label=r'$\pm p_0$')
p0_line2 = ax2.axvline(p0, c='k', ls=':')
...
def init():
    ...
    psi_p_line.set_data([], [])
    title.set_text("")
    return (psi_x_line, V_x_line, center_line, psi_p_line, title)
